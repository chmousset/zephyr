...
LOG_MODULE_REGISTER(allwinner_d1_ccu, CONFIG_CLOCK_CONTROL_LOG_LEVEL);

struct allwinner_d1_ccu_config {
	uintptr_t base;
	uint32_t hosc_rate;
};

struct allwinner_d1_ccu_clk {
	uint16_t id;
...
#define ALLWINNER_D1_CCU_NO_RESET 0xFF

static const struct allwinner_d1_ccu_clk allwinner_d1_ccu_clks[] = {
	...
	{CLK_SPI0, 0x0940, 31, 0, ALLWINNER_D1_CCU_NO_RESET},
	{CLK_SPI1, 0x0944, 31, 0, ALLWINNER_D1_CCU_NO_RESET},
...
static const struct allwinner_d1_ccu_clk *allwinner_d1_ccu_clk_get(uint32_t id)
{
	for (size_t i = 0; i < ARRAY_SIZE(allwinner_d1_ccu_clks); i++) {
		if (allwinner_d1_ccu_clks[i].id == id) {
			return &allwinner_d1_ccu_clks[i];
		}
	}
...
static inline void allwinner_d1_ccu_set_bit(uintptr_t addr, uint8_t bit, bool set)
{
...
static enum clock_control_status allwinner_d1_ccu_get_status(const struct device *dev,
							     clock_control_subsys_t sys)
{
	const struct allwinner_d1_ccu_config *config = dev->config;
	uint32_t clkid = (uint32_t)(uintptr_t)sys;
	const struct allwinner_d1_ccu_clk *clk = allwinner_d1_ccu_clk_get(clkid);
...
static int allwinner_d1_ccu_on(const struct device *dev, clock_control_subsys_t sys)
{
	const struct allwinner_d1_ccu_config *config = dev->config;
	uint32_t clkid = (uint32_t)(uintptr_t)sys;
	const struct allwinner_d1_ccu_clk *clk = allwinner_d1_ccu_clk_get(clkid);
	...
	if (clk->reset_bit != ALLWINNER_D1_CCU_NO_RESET &&
...
static int allwinner_d1_ccu_off(const struct device *dev, clock_control_subsys_t sys)
{
	const struct allwinner_d1_ccu_config *config = dev->config;
	uint32_t clkid = (uint32_t)(uintptr_t)sys;
	const struct allwinner_d1_ccu_clk *clk = allwinner_d1_ccu_clk_get(clkid);
	...
	allwinner_d1_ccu_set_bit(config->base + clk->gate_offset, clk->gate_bit, false);

	return 0;
}

static int allwinner_d1_ccu_get_rate(const struct device *dev,
				     clock_control_subsys_t sys,
				     uint32_t *rate)
{
	const struct allwinner_d1_ccu_config *config = dev->config;
...
static int allwinner_d1_ccu_set_rate(const struct device *dev,
				     clock_control_subsys_t sys,
...
static DEVICE_API(clock_control, allwinner_d1_ccu_api) = {
	.on = allwinner_d1_ccu_on,
	.off = allwinner_d1_ccu_off,
	.get_status = allwinner_d1_ccu_get_status,
	.get_rate = allwinner_d1_ccu_get_rate,
	.set_rate = allwinner_d1_ccu_set_rate,
};

static int allwinner_d1_ccu_init(const struct device *dev)
{
	ARG_UNUSED(dev);
	return 0;
}

static const struct allwinner_d1_ccu_config allwinner_d1_ccu_config = {
	.base = DT_INST_REG_ADDR(0),
	.hosc_rate = DT_INST_PROP_OR(0, clock_frequency, 24000000U),
};

DEVICE_DT_INST_DEFINE(0, allwinner_d1_ccu_init, NULL,
		      NULL, &allwinner_d1_ccu_config,
		      PRE_KERNEL_1, CONFIG_CLOCK_CONTROL_INIT_PRIORITY,
		      &allwinner_d1_ccu_api);
